...
class AnalizaPagos{
	...
	private function separaPolizas(){
		...
		foreach ($this->documentos as $registro) {
			...
			$this->docsAnalisis[$poliza][] = array("poliza" => $registro['poliza'],
													"endoso" => $registro['endoso']
												);
		}
	}

	function doAnalizaPatronesAlerta(){
	...
	private function getDetallePagosEfectivo(){
		$this->detallePagosEfectivo = array();
	...
	private function getDetallePagos(){
		$this->detallePagosFormato = array();
	...
	private function validaRegistroInusualidad(){
		...
		foreach ($this->documentos as $registro) {
			...
			if($result->_numOfRows > 0){
				$this->registrosInusuales[] = array("poliza" => $registro['poliza'],
													"endoso" => $registro['endoso'],
