...
<template>
  <AppLayout title="Perfil Transaccional">
    ...
    <transition name="fade-slide">
      <div
        v-if="mostrarFlash && modalFlashTitle"
        class="mb-6 rounded-xl border px-4 py-3.5 shadow-lg backdrop-blur-sm"
        :class="
          alertaLocal?.tipo === 'success' || flashSuccess
    ...
    >
      ...
      <div class="p-6">
        <!-- Filters Section -->
        <div class="mb-6 flex flex-col gap-4 border-b border-gray-200/60 pb-6 dark:border-neutral-800/60 sm:flex-row sm:items-end sm:justify-between">
          <div class="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
            <label
              class="text-sm font-semibold tracking-tight text-gray-700 dark:text-neutral-300 sm:w-20"
            >
              Periodo:
            </label>
            <select
              v-model="formRegistrar['Periodo']"
            ...
            >
              ...
              <option
                v-for="(p, i) in props.periodos"
                :key="i"
                :value="p.FechaEjecucción"
              >
                {{ p.PeriodoFormateado }}
              </option>
            </select>
        ...
        <div v-if="resultados.length" class="overflow-hidden">
          ...
          <div class="overflow-x-auto rounded-xl">
            <table class="min-w-full divide-y divide-gray-200/60 dark:divide-neutral-800/60">
              <thead class="bg-gradient-to-r from-gray-50/95 to-gray-50/80 backdrop-blur-sm dark:from-neutral-900/95 dark:to-neutral-900/80">
                <tr>
                  <th
                    scope="col"
              ...
              <tbody class="divide-y divide-gray-200/60 bg-white dark:divide-neutral-800/60 dark:bg-neutral-950">
                <tr
                  v-for="(fila, index) in paginatedResults"
                  :key="index"
                ...
                >
                  ...
                  <td class="whitespace-nowrap px-5 py-4 text-right text-sm font-medium text-gray-900 dark:text-neutral-100">
          ...
          <!-- Paginación -->
          <div
            v-if="resultadosFiltrados.length > itemsPerPage"
          ...
          >
            ...
            <div class="flex items-center gap-2 text-sm text-gray-700 dark:text-neutral-300">
              ...
              <span class="rounded-lg bg-gradient-to-r from-gray-100 to-gray-50 px-2.5 py-1 font-bold text-gray-900 shadow-sm dark:from-neutral-800 dark:to-neutral-900 dark:text-neutral-100">
                {{ startIndex }}
              </span>
        ...
        <!-- Empty State -->
        <div
          v-if="!resultados.length && !loading"
      ...
      >
        ...
        <transition name="modal-scale">
          ...
          >
            <div class="flex items-center justify-between">
              <h2
                id="execute-modal-title"
                class="text-lg font-semibold tracking-tight text-gray-900 dark:text-neutral-100"
              >
                Ejecutar Perfil Transaccional
              </h2>
              <button
                @click="closeModalEjecutar"
                type="button"
                class="group rounded-xl p-2 text-gray-400 transition-all duration-200 hover:bg-gray-100 hover:text-gray-600 hover:scale-110 active:scale-95 focus:outline-none focus:ring-2 focus:ring-blue-500/50 dark:hover:bg-neutral-800 dark:hover:text-neutral-300 dark:focus:ring-blue-400/50"
                aria-label="Cerrar"
              >
        ...
        >
          <div class="flex flex-col items-center gap-5">
            <div class="relative">
              ...
              <div class="absolute inset-0 h-10 w-10 animate-ping rounded-full bg-blue-600/20 dark:bg-blue-400/20"></div>
            </div>
